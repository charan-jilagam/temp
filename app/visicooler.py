    ...
    try:
        ...
        for col, props in required_columns.items():
            if col not in columns:
                logger.error(f"Column {col} not found in orgi.visibilitydetails. Please add it with: ALTER TABLE orgi.visibilitydetails ADD COLUMN {col} {props['expected_types'][0]};")
                return False
            if columns[col] not in props['expected_types']:
                logger.error(f"Column {col} has unexpected data type: {columns[col]}. Expected one of {props['expected_types']}. Please alter with: ALTER TABLE orgi.visibilitydetails ALTER COLUMN {col} TYPE {props['expected_types'][0]};")
                return False
        return True
    ...
    try:
        ...
        for record in records:
            ...
            if not isinstance(present_no_facings, str):
                logger.warning(f"Invalid present_no_facings type for {filename}: {type(present_no_facings)}. Using 'N'.")
        ...
        conn.commit()
        logger.info(f"Uploaded {len(formatted_records)} records to orgi.visibilitydetails with cyclecountid {cyclecountid}")
    except Exception as e:
...
def run_visicooler_analysis(image_paths, config, s3_handler, conn, cur, output_folder_path, cyclecountid):
    ...
    try:
        shelf_model_path = config['visicooler_config']['shelf_model_path']
        ...
        coca_cola_classes = {
            'Coca-Cola 1000ml PET', 'Coca-Cola 1500ml PET', 'Coca-Cola 2250ml PET', 'Coca-Cola 250ml Glass',
            'Coca-Cola 250ml PET', 'Coca-Cola 500ml PET', 'Sprite 1500ml PET', 'Sprite 2250ml PET',
            'Sprite 250ml PET', 'Sprite 500ml PET', 'Sprite 250ml Glass', 'Fanta Orange 500ml PET',
            'Fanta Orange 250ml PET', 'Fanta Orange 1000ml PET', 'Fanta Orange 1500ml PET',
            'Fanta Orange 250ml Glass', 'Fanta Apple 500ml PET', 'Charged Energy Drink 250ml PET'
        }

        pepsico_classes = {
            'Pepsi 1000ml PET', 'Pepsi 1500ml PET', 'Pepsi 2250ml PET', 'Pepsi 250ml Glass', 'Pepsi 250ml PET',
            'Pepsi 500ml PET', 'Pepsi 600ml PET', 'Pepsi 700ml PET', 'Pepsi Visicooler',
            'Mountain Dew 1000ml PET', 'Mountain Dew 1250ml PET', 'Mountain Dew 1500ml PET',
            'Mountain Dew 2250ml PET', 'Mountain Dew 250ml Glass', 'Mountain Dew 250ml PET',
            'Mountain Dew 500ml PET', 'Mountain Dew 600ml PET', 'Mountain Dew 750ml PET',
            'Mirinda Orange 1000ml PET', 'Mirinda Orange 1500ml PET', 'Mirinda Orange 2250ml PET',
            'Mirinda Orange 250ml Glass', 'Mirinda Orange 250ml PET', 'Mirinda Orange 500ml PET',
            'Slice 1000ml PET Bottle', 'Slice 250ml Glass', 'Slice 250ml PET', 'Slice 500ml PET',
            '7Up 250ml Glass Bottle', '7Up Nimbooz 2250ml PET', '7Up Nimbooz 250ml PET',
            '7Up Nimbooz 500ml PET', '7up 250ml PET', '7up 250ml glass', '7up 500ML PET',
            'Aquafina 1000ml PET', 'Aquafina 250ml', 'Aquafina 500ml PET', 'Sting 250ml PET',
            'Teem Soda 250ml PET'
        }
            ...
            try:
                ...
                for region in shelf_regions:
                    ...
                    if skus:
                        companies = set()
                        for sku in skus:
                            if sku["name"] in coca_cola_classes:
                                companies.add("coca")
                            elif sku["name"] in pepsico_classes:
                                companies.add("pepsi")
                            else:
                                companies.add("other")
                        if len(companies) == 1 and ("coca" in companies or "pepsi" in companies):
                            pure_shelf_count += 1
                ...
                total_products = len(sku_detections)
